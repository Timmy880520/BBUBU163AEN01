...
History:
    ...
    04/29/2021 Watch Lee         1. version 1.2.1.
                                 2. add Boot_Internal_Single_Step_Init function.
...
EnumBootCommand Boot_Unlocked_State_Machine(BootInternalObj *obj, BootInternalObj *transportObj);
...
Input:
    *obj        - Object pointer of boot internal task.
    waiveHeader - Flag of waiving header. 1 = waive header verification. 0 = execute header verification.
                                       ...
                                       unsigned long headerStart, unsigned long appRomSize)
{
    ...
    unsigned long dataCRC = Get_Package_CRC32(0xFFFFFFFF, (unsigned char*)&(appHeader.header),
    ...
    else if (obj->member.romHeaderPtr)
    {
        ...
        if (appHeader.header.programFunction.word == obj->member.ownFunctionId.word &&
            appHeader.header.programSize > sizeof(appHeader) &&
            appHeader.header.programSize <= (appRomSize * 2) &&
            obj->member.checksum.calculate)
        {
            // Update CRC of app code
            unsigned long crcStart = headerStart + (sizeof(appHeader) >> 1);
            unsigned char buffer[Boot_Buffer_Size];

            dataCRC = obj->member.checksum.seed;

            for (long i = 0; i < (appHeader.header.programSize - sizeof(appHeader)) >> 1; i += Boot_Buffer_Size >> 1)
            {
                TiByte_Convert((char*)crcStart + i, buffer, Boot_Buffer_Size >> 1);
                dataCRC = obj->member.checksum.calculate(dataCRC, buffer, Boot_Buffer_Size);
            }
        }
        else 
            dataCRC = ~appHeader.signature.imageCrc;

        // Check CRC of app code	
        if (dataCRC == appHeader.signature.imageCrc)
            return BOOT_LOCKED;
        else 
            obj->member.message.state.stateCode = (BOOT_ERROR << 8) + CHECKSUM_ERROR;	    
    }	
    else
...
/**************************************************************************************************
Function Name:
    EnumBootState Boot_Internal_Single_Step_Init(BootInternalObj *obj, unsigned char waiveHeader,
                                                 unsigned long headerStart, unsigned long appRomSize)
Input:
    *obj        - Object pointer of boot internal task.
    waiveHeader - Flag of waiving header. 1 = waive header verification. 0 = execute header verification.
...
Output:
    return      - Boot state code.
Comment:
    Step init boot state function. This function will verify the header information if waiveHeader flag
is zero. This function must keep executing until the checksum calculating is completed.
**************************************************************************************************/
EnumBootState Boot_Internal_Single_Step_Init(BootInternalObj *obj, unsigned char waiveHeader,
                                             unsigned long headerStart, unsigned long appRomSize)
{
    ...
    else if (obj->member.romHeaderPtr)
    {
            ...
            obj->member.checksum.calculate)
        {
            ...
            if (byteCount < ((appHeader.header.programSize - sizeof(appHeader)) >> 1))
            {
                TiByte_Convert((char*)crcStart + byteCount, buffer, Boot_Buffer_Size >> 1);
                crcStep = obj->member.checksum.calculate(crcStep, buffer, Boot_Buffer_Size);
                byteCount += Boot_Buffer_Size >> 1;
            ...
            else
            {
                ...
                if (crcStep == appHeader.signature.imageCrc)
                    return BOOT_LOCKED;
                else
                    obj->member.message.state.stateCode = (BOOT_ERROR << 8) + CHECKSUM_ERROR;
            }
...
EnumBootCommand Boot_Unlocked_State_Machine(BootInternalObj *obj, BootInternalObj *transportObj)
{
    switch (obj->member.message.state.byte.subState)
    {
        ...
        case PROCESSING:
            ...
            if (obj->member.argument.mode == TRANSPORT_MODE || obj->member.argument.mode == PROGRAM_MODE)
            {	                          
                if (obj->member.argument.mode == PROGRAM_MODE && obj->method.program)               
                {
                    unsigned long offset = Boot_Buffer_Size * obj->member.argument.blockOffset;
                    
                    if (obj->method.program(offset, obj->member.argument.buffer, Boot_Buffer_Size))
                    {
                        obj->member.message.state.byte.subState = READY;
                        return BOOT_DOWNLOAD;
                    }
                    else
                        obj->member.message.state.stateCode = (BOOT_ERROR << 8) + FLASH_ERROR;
                }
                else if (obj->member.argument.mode == TRANSPORT_MODE && obj->method.transport)               
                {
                    ArrayType buffer = {obj->member.argument.buffer, Boot_Buffer_Size};
                    EnumErrorCode errorCode = obj->method.transport(transportObj, buffer, 0);
                    ...
                    {    
                        ...
                        obj->member.message.state.stateCode = (BOOT_TRANSPORT_ERROR << 8) + errorCode;
                }
                else
                    obj->member.message.state.stateCode = (BOOT_ERROR << 8) + UNKNOWN_ERROR;
            }              
        ...
        case VERIFYING:
            if (obj->member.argument.transportEnable && obj->method.transport)
            {
                ...
                EnumErrorCode errorCode = obj->method.transport(transportObj, buffer, 1);
                
                if (errorCode == NO_ERROR)
                    obj->member.message.state.byte.subState = VERIFIED;
                else if (errorCode < PENDING)
                    obj->member.message.state.stateCode = (BOOT_ERROR << 8) + errorCode;
            }
            else
                obj->member.message.state.byte.subState = VERIFIED;
            break;
...
unsigned char Check_Boot_Protection(BootInternalObj *obj)
{
    ...
    {    
        if (obj->member.message.state.byte.bootState == BOOT_LOCKED)
            obj->member.message.state.stateCode = (BOOT_UNLOCKED << 8) + PREPARING;
        obj->member.message.protectKey = 0;
        return 1;
    }
    return 0;
...
unsigned char Boot_Download_Process(BootInternalObj *obj)
{
    ...
    obj->member.argument.accumulation = obj->member.argument.blockCount * Boot_Buffer_Size + 
...
void Update_Boot_Buffer(BootInternalObj *obj)
{
    ...
    {
        obj->member.argument.buffer[obj->member.argument.byteCount & (Boot_Buffer_Size - 1)] = 
...
EnumErrorCode Check_Boot_Buffer(BootInternalObj *obj)
{
    ...
    {	      
        ...
        {
            case 0:
                result = Configure_Boot_Mode(obj, obj->member.appHeader.header.programFunction.word,
                                             obj->member.appHeader.header.programSize);
                break;
            case 1:
                result = Configure_Boot_Mode(obj, obj->member.mixHeader.firstIndex.programFunction.word,
                                             obj->member.mixHeader.firstIndex.programSize);
                break;
            case 2:
                result = Configure_Boot_Mode(obj, obj->member.mixHeader.secondIndex.programFunction.word,
                                             obj->member.mixHeader.secondIndex.programSize);
                break;
            case 3:
                result = Configure_Boot_Mode(obj, obj->member.mixHeader.thirdIndex.programFunction.word,
                                             obj->member.mixHeader.thirdIndex.programSize);
                break;
            case 4:
                result = Configure_Boot_Mode(obj, obj->member.mixHeader.fourthIndex.programFunction.word,
                                             obj->member.mixHeader.fourthIndex.programSize);
                break;
            case 5:
                subHeaderPtr = (&obj->member.mixHeader.secondIndex + obj->member.argument.headerNumber - 2);
                result = Configure_Boot_Mode(obj, subHeaderPtr->programFunction.word, subHeaderPtr->programSize);
                break;                  
            default:
...
EnumErrorCode Check_Header_Message(BootInternalObj *obj)
{
    ...
    if (!obj->member.argument.blockCount)
    {
        ...
        else if (*(long long*)&obj->member.argument.buffer == *(long long*)"APP")
        {
    ...
    else
    {
        if (obj->member.argument.mode != GET_MIX_HEADER)
        {
            ...
            if (Boot_Buffer_Size < 64)
                ...
                functionId = Correct_Header_Data(obj->member.appHeader.header.programFunction.word);
            result = Configure_Boot_Mode(obj, functionId, sizeof(StdMixHeaderType));
        }
...
void Update_Header_Member(BootInternalObj *obj, void *headerPtr)
{
    ...
    {	
        *((unsigned char*)headerPtr + obj->member.argument.blockCount * Boot_Buffer_Size + i) = 
...
EnumErrorCode Configure_Boot_Mode(BootInternalObj *obj, unsigned long funcId, unsigned long size)
{
    ...
    obj->member.argument.blockOffset = obj->member.argument.blockCount - obj->member.argument.startBlock;
                        
    if (size == (Boot_Buffer_Size * obj->member.argument.blockOffset + obj->member.argument.byteCount))
    {
        obj->member.argument.startBlock = 0xFFFFFFFF;
        return NO_ERROR;
    }
    else if (size > (Boot_Buffer_Size * obj->member.argument.blockOffset + obj->member.argument.byteCount)) 
        return NO_ERROR;
    else 
        return SIZE_ERROR;	
}
...
{   
    ...
    {                       
        if (obj->member.argument.mode == GET_MIX_HEADER)
        {    
            if (Compare_Module_Name(obj, obj->member.mixHeader.firstIndex.moduleName) < 
                sizeof(obj->member.romHeaderPtr->header.moduleName))
                return 0;
        }
        else
        {
            if (Compare_Module_Name(obj, obj->member.appHeader.header.moduleName) < 
                sizeof(obj->member.romHeaderPtr->header.moduleName))
                return 0;                     
        }
    }
...
unsigned short Compare_Module_Name(BootInternalObj *obj, unsigned char *namePtr)
{
    ...
    {
        ArrayType compliant = {(unsigned char*)(long)obj->member.romHeaderPtr->compliantTableAddress, size};
        unsigned long lastTable = obj->member.romHeaderPtr->compliantTableAddress + 10 * size;  // support 10 models
...
{    
    if (obj->member.argument.accumulation == sizeof(obj->member.mixHeader))
    {
        ...
        long mixCRC = Get_Package_CRC32(0xFFFFFFFF, (unsigned char*)&obj->member.mixHeader.firstIndex, 
                                        sizeof(obj->member.mixHeader) - sizeof(obj->member.mixHeader.signature));                

        // Checksum correction
        long crcData = Correct_Header_Data(obj->member.mixHeader.signature.headerCrc);
...
{ 
    ...
    {    
        if (!finish)
        {
            ...
            {
                unsigned char *appHeaderPtr = (unsigned char*)&obj->member.appHeader + 
                                              obj->member.argument.blockCount * Boot_Buffer_Size;
            ...
            if (obj->member.argument.byteCount < Boot_Buffer_Size)
            {    
                for (int i = 0; i < obj->member.message.download.member.size; i++)
                    obj->member.message.download.byte[i] = data.byte[obj->member.argument.byteCount + i];
                obj->member.argument.byteCount += obj->member.message.download.member.size;
...
void Correct_Mix_Header_Information(BootInternalObj *obj)
{
    ...
    obj->member.mixHeader.signature.headerCrc =
            Correct_Header_Data(obj->member.mixHeader.signature.headerCrc);

    obj->member.mixHeader.signature.imageCrc =
            Correct_Header_Data(obj->member.mixHeader.signature.imageCrc);

    obj->member.mixHeader.signature.imageSize =
            Correct_Header_Data(obj->member.mixHeader.signature.imageSize);

    // First index
    obj->member.mixHeader.firstIndex.programFunction.word =
        Correct_Header_Data(obj->member.mixHeader.firstIndex.programFunction.word);

    obj->member.mixHeader.firstIndex.programSize =
        Correct_Header_Data(obj->member.mixHeader.firstIndex.programSize);

    // Second index
    obj->member.mixHeader.secondIndex.programFunction.word =
        Correct_Header_Data(obj->member.mixHeader.secondIndex.programFunction.word);

    obj->member.mixHeader.secondIndex.programSize =
        Correct_Header_Data(obj->member.mixHeader.secondIndex.programSize);

    // Third index
    obj->member.mixHeader.thirdIndex.programFunction.word =
        Correct_Header_Data(obj->member.mixHeader.thirdIndex.programFunction.word);

    obj->member.mixHeader.thirdIndex.programSize =
        Correct_Header_Data(obj->member.mixHeader.thirdIndex.programSize);

    // Fourth index
    obj->member.mixHeader.fourthIndex.programFunction.word =
        Correct_Header_Data(obj->member.mixHeader.fourthIndex.programFunction.word);

    obj->member.mixHeader.fourthIndex.programSize =
        Correct_Header_Data(obj->member.mixHeader.fourthIndex.programSize);

    // Fifth index
    obj->member.mixHeader.fifthIndex.programFunction.word =
        Correct_Header_Data(obj->member.mixHeader.fifthIndex.programFunction.word);

    obj->member.mixHeader.fifthIndex.programSize =
        Correct_Header_Data(obj->member.mixHeader.fifthIndex.programSize);
}
/**************************************************************************************************
Function Name:
...
{
    // Signature
    headerPtr->signature.headerCrc = Correct_Header_Data(headerPtr->signature.headerCrc);
    headerPtr->signature.imageCrc = Correct_Header_Data(headerPtr->signature.imageCrc);

    // Header
    headerPtr->header.programFunction.word = Correct_Header_Data(headerPtr->header.programFunction.word);
    headerPtr->header.programSize = Correct_Header_Data(headerPtr->header.programSize);

    // Compliant Table
    headerPtr->compliantTableAddress = Correct_Header_Data(headerPtr->compliantTableAddress);
}
/**************************************************************************************************
Function Name:
    unsigned long Correct_Header_Data(unsigned long long raw)
...
unsigned long Correct_Header_Data(unsigned long long raw)
{
    return (raw & 0xFF) | ((raw >> 8) & 0xFF00) | ((raw >> 16) & 0xFF0000) | ((raw >> 24) & 0xFF000000);
}
//---------------- END LINE -----------------------------------------------------------------------
