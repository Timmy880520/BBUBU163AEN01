/**************************************************************************************************
File Name: Boot_Internal_Task.c
External Data:
    Name                                    Source
    --------------------------------------- -----------------------------------------------------
    None
...
Description:  
        Boot internal task program.

===================================================================================================
History:
    Date       Author           Description of Change
    ---------- ---------------- -----------------------------------------------------------------
    05/06/2019 Watch Lee        1. version 1.0.
    11/08/2019 Watch Lee        1. version 1.1.
                                2. modify Check_Header_Message function to support 
                                   Boot_Buffer_Size < 32 byte.
                                3. add Boot_Buffer_Size limitation, it cannot set over 128 byte.
    ...
    10/08/2021 Watch Lee        1. version 1.3.
    ...
    11/01/2021 Watch Lee        1. version 1.4.
...
#include "..\inc\Boot_Internal_Task.h"
...
EnumBootState Boot_Internal_State_Init(BootInternalObj *obj, unsigned char feature,
                                       unsigned long headerStart, unsigned long appRomSize)
{
    StdAppHeaderType *appHeaderPtr = (StdAppHeaderType*)headerStart;
    
    unsigned long dataCRC = Get_Package_CRC32(0xFFFFFFFF, (unsigned char*)&(appHeaderPtr)->header, 
                                              sizeof(*appHeaderPtr) - sizeof(appHeaderPtr->signature));
    
    if (appHeaderPtr->signature.headerCrc == dataCRC)
        obj->member.romHeaderPtr = appHeaderPtr;
    else
    ...
    else if (obj->member.romHeaderPtr)
    {
        ...
        if (appHeaderPtr->header.programFunction.word == obj->member.ownFunctionId.word && 
            appHeaderPtr->header.programSize > sizeof(*appHeaderPtr) && 
            appHeaderPtr->header.programSize <= appRomSize && 
            obj->member.checksum.calculate)
        {
            // Update CRC of app code
            unsigned long crcStart = (unsigned long)obj->member.romHeaderPtr + sizeof(*obj->member.romHeaderPtr);
            unsigned long stepSize = appHeaderPtr->header.programSize - sizeof(*appHeaderPtr);

            if ((feature & BOOT_STEP_INIT) == BOOT_STEP_INIT)
                stepSize = Boot_Buffer_Size;
...
/**************************************************************************************************
Function Name:
    EnumBootState Boot_Image_CRC_Calculation(BootInternalObj *obj, unsigned long stepSize)
Input:
...
EnumBootState Boot_Image_CRC_Calculation(BootInternalObj *obj, unsigned long crcStart, unsigned long stepSize)
{
    unsigned long appSize = obj->member.romHeaderPtr->header.programSize - sizeof(*obj->member.romHeaderPtr);
    ...
    {
        crcStep = obj->member.checksum.calculate(crcStep, (char*)crcStart, stepSize);
        byteCount += stepSize;
        return BOOT_INITIATING;
    }
    else if (byteCount < appSize)
    {
        crcStep = obj->member.checksum.calculate(crcStep, (char*)crcStart, appSize - byteCount);
        byteCount = 0;
...
/*    if ((byteCount + stepSize) < appSize)
    ...
    {
        stepSize = appSize - byteCount;
        byteCount = 0;
    }
    crcStep = obj->member.checksum.calculate(crcStep, (char*)crcStart + byteCount, stepSize);
            ...
            else if (obj->member.message.completeKey == BOOT_COMPLETE_KEY)
            {
                ...
                if (obj->member.argument.accumulation & (Boot_Buffer_Size - 1))
                {
                    if (Check_Boot_Buffer(obj) == NO_ERROR)
                        ...
                        obj->member.message.state.stateCode = (BOOT_ERROR << 8) + Check_Boot_Buffer(obj);
                }
                else
                {
        ...
        case PROCESSING:
            ...
            {	                          
                if (obj->member.argument.mode == PROGRAM_MODE && obj->method.program)               
                {
                    ...
                    unsigned long size = obj->member.argument.accumulation & (Boot_Buffer_Size - 1);

                    size = (!size) ? Boot_Buffer_Size : size;

                    if (obj->method.program(offset, obj->member.argument.buffer, size))
                    {
        ...
        switch(obj->member.argument.headerNumber)
        {
            case 0:
                if (obj->member.argument.headerCheckDisable)
                    result = Configure_Boot_Mode(obj, obj->member.ownFunctionId.word, 0xFFFFFFFF);
                else
                {
...
EnumErrorCode Check_Header_Message(BootInternalObj *obj)
{
    ...
    {
        if (*(long*)&obj->member.argument.buffer == *(long*)"MIX")
        {    
            obj->member.argument.mode = GET_MIX_HEADER;
            Update_Header_Member(obj, &obj->member.mixHeader);
        }
        else if (*(long*)&obj->member.argument.buffer == *(long*)"APP" || obj->member.argument.headerCheckDisable)
        {
    ...
    else
    {
        if (obj->member.argument.mode != GET_MIX_HEADER)
        {
            ...
            if (Boot_Buffer_Size < 64 || obj->member.argument.headerCheckDisable)
                functionId = obj->member.ownFunctionId.word;
            else
                functionId = obj->member.appHeader.header.programFunction.word;
            result = Configure_Boot_Mode(obj, functionId, sizeof(StdMixHeaderType));
        }

        if (!obj->member.argument.blockCount && Boot_Buffer_Size >= 32)
            result = (Check_Module_Name(obj) || obj->member.argument.headerCheckDisable) ? result : HEADER_ERROR;
    }
    return result;
}
/**************************************************************************************************
...
unsigned short Compare_Module_Name(BootInternalObj *obj, unsigned char *namePtr)
{
    ...
    {
        ArrayType compliant = {(unsigned char*)obj->member.romHeaderPtr->compliantTableAddress, size};
...
{    
    if (obj->member.argument.accumulation == sizeof(obj->member.mixHeader))
    {
        ...
        if (obj->member.mixHeader.signature.headerCrc != mixCRC)
            return HEADER_ERROR;
        else
            obj->member.argument.startBlock = 0xFFFFFFFF;
    }
